...
type Repository struct {
	db             *sql.DB
	filteredStmtMu sync.Mutex
	filteredStmts  map[string]*sql.Stmt
}
...
func New(db *sql.DB) *Repository {
	return &Repository{db: db, filteredStmts: make(map[string]*sql.Stmt)}
}
...
func (r *Repository) ListFiltered(ctx context.Context, f CaseFilter) ([]domain.QuarantineCase, int, error) {
	...
	stmt, err := r.filteredStatement(ctx, q)
	if err != nil {
		return nil, 0, err
	}
	defer stmt.Close()
...
func (r *Repository) filteredStatement(ctx context.Context, query string) (*sql.Stmt, error) {
	r.filteredStmtMu.Lock()
	defer r.filteredStmtMu.Unlock()
	if stmt, ok := r.filteredStmts[query]; ok {
		return stmt, nil
	}
