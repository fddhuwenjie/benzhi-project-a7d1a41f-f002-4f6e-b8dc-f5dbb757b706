...
func (s *Service) AddObservation(ctx context.Context, caseID string, input AddObservationInput) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, input.ExpectedRevision, input.RequestID, "observation.recorded", input.Actor, now, func(a *domain.CaseAggregate) (any, error) {
...
func (s *Service) ReviewLateObservation(ctx context.Context, caseID, observationID string, input ReviewLateObservationInput) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, input.ExpectedRevision, input.RequestID, "observation.late_reviewed", input.Actor, now, func(a *domain.CaseAggregate) (any, error) {
