...
func (s *Service) SubmitRisk(ctx context.Context, caseID string, input SubmitRiskInput) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, input.ExpectedRevision, input.RequestID, "risk.submitted", input.Actor, now, func(a *domain.CaseAggregate) (any, error) {
...
func (s *Service) ReviewRisk(ctx context.Context, caseID string, input ReviewInput) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, input.ExpectedRevision, input.RequestID, "risk.reviewed", input.Actor, now, func(a *domain.CaseAggregate) (any, error) {
		if a.Risk == nil {
...
func (s *Service) StartObservation(ctx context.Context, caseID string, meta Meta) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, meta.ExpectedRevision, meta.RequestID, "observation.started", meta.Actor, now, func(a *domain.CaseAggregate) (any, error) {
