...
func (s *Service) PreviewEligibility(ctx context.Context, caseID string) (EligibilityView, error) {
	...
	cached, ok := s.eligibilityPreviews[caseID]
	...
	view := EligibilityView{Result: result, CaseRevision: agg.Case.Revision, Windows: windows}
	s.eligibilityMu.Lock()
	s.eligibilityPreviews[caseID] = view
