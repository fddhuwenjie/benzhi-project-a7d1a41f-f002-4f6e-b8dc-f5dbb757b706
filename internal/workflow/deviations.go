...
func (s *Service) OpenDeviation(ctx context.Context, caseID string, input OpenDeviationInput) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, input.ExpectedRevision, input.RequestID, "deviation.opened", input.Actor, now, func(a *domain.CaseAggregate) (any, error) {
...
func (s *Service) VerifyDeviation(ctx context.Context, caseID, deviationID string, input VerifyDeviationInput) (Envelope[domain.CaseAggregate], error) {
	...
	now := s.clock.Now()
	result, err := s.repo.Mutate(context.Background(), caseID, input.ExpectedRevision, input.RequestID, "deviation.verified", input.Actor, now, func(a *domain.CaseAggregate) (any, error) {
